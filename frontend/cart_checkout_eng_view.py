...
from cart.forms import CheckOutEngForm
...
def add_product_to_cart(request, slug):
    ...
    if not product.product_class.have_transcations:
        messages.warning(request, f'The product {product.eng_title} doesnt support transcations.')
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    ...
    if check_cart_owner:
        ...
        cart_item.save()
        messages.success(request, f'The product {product.eng_title} added to your cart!')
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
class CheckoutView(FormView):
    ...
    def form_valid(self, form):
        ...
        if not cart.order_items.exists():
            messages.warning(self.request, 'You have to add items to your cart first.')
